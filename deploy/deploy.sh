...
LC_CTYPE=C && find ./resources/ -type f -exec sed -i '' "s/\[PROJECT_ID\]/${PROJECT_ID}/g" {} \;
...
gcloud projects add-iam-policy-binding ${PROJECT_ID} --member "serviceAccount:${WP_SA_EMAIL}" --role roles/cloudsql.client


# change project id back
LC_CTYPE=C && find ./resources/ -type f -exec sed -i '' "s/${PROJECT_ID}/\[PROJECT_ID\]/g" {} \;
